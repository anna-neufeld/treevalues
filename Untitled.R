...
splits <- getAncestors(base_tree, 11)
  ...
  nu <- (base_tree$where ==11)/sum(base_tree$where==11) -  (base_tree$where==11)/sum(base_tree$where==11)
  ...
  for (i in 1:length(splits)) {
    ...
    num22s <- apply(c_now, 1, function(u) sum((u+nulled)==1))
    num2s <- rowSums(nulled-c_now)
    
    all.equal(denoms, num1s*(num2s)/(num1s+num2s))
