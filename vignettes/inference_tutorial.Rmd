...
The arguement ``cp``, or complexity parameter, is a scaled version of the complexity parameter $\lambda$ described in our manuscript. For response variable $y$, $\lambda =\text{cp} \times \sum_{i=1}^n (y_i-\bar{y})^2$. The larger the value of ``cp``, the more the tree will be pruned. 
...
In our framework, it is assumed that $y_i \sim N(\mu_i, \sigma^2)$ and $\sigma^2$ is assumed known. If no arguement ``sigma_y`` is provided to the inference function, the conservative estimate $sd(y)$ is used.

# Inference on a single region

Suppose that we instead just want a confidence interval for the mean result within region 8. We simply change the ``type`` arguement. 
...
There are many ways to customize this plot. If the default version is too congested, alternate values of ``inferenceType`` can be provided to customize how much information is displayed. Addional arguements provided will be passed on to ```rpart.plot()``. For example,

```{r,dpi=300, out.width="90%"}
treeval.plot(bls.tree, inferenceType=1, cex=0.5, printn=FALSE)
```
