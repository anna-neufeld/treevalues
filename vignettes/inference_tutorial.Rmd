...
While this seems like an argument for always setting ``permute=TRUE``, we note that computations can be prohibively slow for large trees when ``permute=TRUE``. We also note that adding ``permute=TRUE`` tends to make a large difference only in trees where the overall signal is weak. Trees with very clear, significant splits tend to be more stable, and in more stable trees the addition of `permute=TRUE`` does not tend to substantially shorten confidence intervals. 
...
There are many ways to customize this plot. If the default version is too congested, alternate values of ``inferenceType`` can be provided to customize how much information is displayed. Addional arguments provided will be passed on to ```rpart.plot()``. For example,
