...
There are many ways to customize the output of ``treeval.plot``. If the default version is too congested, alternate values of ``inferenceType`` can be provided to customize how much information is displayed. Addional arguments provided will be passed on to ```rpart.plot()``. 
...
treeval.plot(bls.tree, inferenceType=2, printn=FALSE, box.col="green")
treeval.plot(bls.tree, inferenceType=1, printn=FALSE, box.col="purple")
