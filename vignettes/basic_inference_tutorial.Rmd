...
The ``treevalues`` package is designed for use with the ``rpart`` package, and so all trees should be built using ``rpart``. All trees should be built with the parameter ``model=TRUE``, which simply saves a copy of the training data inside of the fitted rpart object (this is useful so that the data need not be separately passed to each inference function). 

```{r, tree}
bls.tree <-rpart(kcal24h0~hunger+disinhibition+resteating+rrvfood+liking+wanting, model = TRUE, data = blsdata, cp=0.02, maxcompete=0, maxsurrogate=0)
```
