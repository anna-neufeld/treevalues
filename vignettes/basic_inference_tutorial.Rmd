...
Note that the arguement ``cp``, or complexity parameter, is a scaled version of the complexity parameter $\lambda$ described in our manuscript, $\lambda = $``cp`` $\times \sum_{i=1}^n (y_i-\bar{y})^2$. 
...
```{r}
treeval.plot(bls.tree,  inference=TRUE,nn=TRUE)
```
