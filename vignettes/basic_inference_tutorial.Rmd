...
In this tutorial, we demonstrate how to use the ``treevalues`` package to perform inference on a CART (@breiman1984classification) tree fit using the ``rpart`` package (@therneau2015package). Throughout this tutorial, we work with an example tree fit to the Box Lunch Study dataset, which is provided in the ``visTree`` package. 

We start by loading the two packages we will be working with, along with the dataset we will be using.

```{r}
#library(treevalues)
#library(rpart)
#data(blsdata, package="visTree")
```

# Building the tree

The ``treevalues`` package is designed for use with the ``rpart`` package, and so all trees should be built using ``rpart``. All trees should be built with the parameter ``model=TRUE``, which simply saves a copy of the training data inside of the fitted rpart object (which is useful when carrying out inference). 

```{r, tree}
#bls.tree <-rpart(kcal24h0~hunger+disinhibition+resteating+rrvfood+liking+wanting, model = TRUE, data = blsdata, cp=0.02)
```

Note that the arguement ``cp``, or complexity parameter, is a scaled version of the complexity parameter $\lambda$ described in our manuscript, $\lambda = $``cp`` $\times \sum_{i=1}^n (y_i-\bar{y})^2$. \textbf{Note that the RPART documentation is wrong}. 

We might first want to plot the tree. While this could be accomplished using ``plot()`` from the ``rpart`` package, or else using something more specialized like the ``rpart.plot`` package, we recommend that you use our internal ``treeval.plot()`` function. However, since we are just exploring our tree and have not yet decided to do inference, you can set ``inference=FALSE``.  

```{r}
#treeval.plot(bls.tree, inference=FALSE)
```
...
```{r}
#result <- treevalues::branchInference(bls.tree, branch, type="sib")
#result$confint
#result$pval
```
...
```{r}
#treeval.plot(bls.tree,  inference=TRUE)
```

 
Or we can see it in a matrix. I need to work on my row and column names for this!!

```{r}
#mat <- fullTreeInference(bls.tree)
#print(mat, ndigits=3)
```
