...
#' @return This function returns a vector of lower and upper confidence limits.
#'
computeCI_Asym <- function(v, y, sigma, truncation, alpha1, alpha2) {
  #browser()
  vTv <- sum(v*v)
  scale <- sigma * sqrt(vTv)
  q <- sum(v*y) / scale
  ...
  # find x1 s.t. fun(x1) <= alpha1
  # what we know:
  # fun is monotone increasing; HOW DO THEY KNOW THIS?? THIS IS IMPORTANT
  # fun(x) = NaN if x too small;
  # fun(x) > alpha/2 if x too big.
  # so we can do a modified bisection search to find x1.
  step <- 0
  ...
  while(step <= 20) {
    ...
    else if (f1 > alpha1) { # x1 is too big
      x1.up <- x1
      x1 <- x1 - 10 * 1.4^step
      f1 <- fun(x1)
      step <- step + 1
  ...
  while(step <= 20) {
    ...
    else if (f2 < alpha2) { # x2 is too small
      x2.lo <- x2
      x2 <- x2 + 10 * 1.4^step
      f2 <- fun(x2)
      step <- step + 1
  ...
  # if the above search does not work, set up a grid search
  # for starting points
  if (is.na(f1)||(f1 > alpha1)||is.na(f2)||(f2 < alpha2)) {
    grid <- seq(from = q * scale - 1000*scale, to = q*scale + 1000*scale)
    value <- sapply(grid, fun)
    # want max x1: fun(x1) <= alpha/2
    ind1 <- rev(which(value <= alpha1))[1]
    x1 <- grid[ind1]
    #f1 = value[ind1]
    # want min x2: fun(x2) >= 1-alpha/2
