...
#' @export
getInterval_full <- function(base_tree, nu, splits) {
  minbucket <- base_tree$control$minbucket
  cp <- base_tree$control$cp
  ...
  if (length(splits) > 1) {
    ...
    vecChildren <- eval(parse(text = paste(splits, collapse=" & ")))
    childrenGains <-   sum((y[as.logical(vecChildren)]-mean(y[as.logical(vecChildren)]))^2)  - sum((y[as.logical(vecChildren)]-predict(base_tree)[as.logical(vecChildren)])^2)
    childrenSize <- length(unique(base_tree$where[as.logical(vecChildren)]))-1  #DO WE MINUS 1???

    inequalities <- sapply(splits, function(u) eval(parse(text = u)))
    ...
    SSE_full_tree <- sum((y-predict(base_tree))^2)

    subgroup_GAINZ <- function(vec) {
      sum((y[as.logical(vec)]-mean(y[as.logical(vec)]))^2)  - sum((y[as.logical(vec)]-predict(base_tree)[as.logical(vec)])^2)
    }

    numSplits <- function(vec) {
      length(unique(base_tree$where[as.logical(vec)])) - 1
    }
    ...
    ### my splits ... more later.
    sizeZ <- apply(off_subgroup_membership, 1, numSplits)
    sizeZfull <- cumsum(sizeZ[length(sizeZ):1])
    } else {

    ### Wait if there is
    off_subgroup_membership <- !eval(parse(text = splits))
    vec <- off_subgroup_membership
    gainZfull <-   sum((y[as.logical(vec)]-mean(y[as.logical(vec)]))^2)  - sum((y[as.logical(vec)]-predict(base_tree)[as.logical(vec)])^2)
    sizeZfull <- length(unique(base_tree$where[as.logical(vec)])) - 1

    vecChildren <- eval(parse(text = splits))
    childrenGains <-   sum((y[as.logical(vecChildren)]-mean(y[as.logical(vecChildren)]))^2)  - sum((y[as.logical(vecChildren)]-predict(base_tree)[as.logical(vecChildren)])^2)
    childrenSize <- length(unique(base_tree$where[as.logical(vecChildren)])) - 1 ### DO I MINUS 1???? no, right?? NOT AGAIN???
  ...
  #### OLD VERSION KINDA WORKED
 # cpMat <- cbind(cumsum(gainsA)/(1:L), cumsum(gainsB)/(1:L), cumsum(gainsC)/(1:L) - cp_cutoff)
...
#' @export
#' @importFrom stats predict
getInterval_prune <- function(base_tree, nu, splits) {
  minbucket <- base_tree$control$minbucket
  cp <- base_tree$control$cp
  if (is.null(base_tree$model)) {
    stop("rpart tree must be built with model=TRUE")
