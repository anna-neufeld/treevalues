...
getInterval_FASTER <- function(base_tree, nu, splits) {
  ...
  for (l in 1:length(splits)) {
    ...
    for (j in 1:p) {
      c_now <- t(apply(cs[j,,], 1, function(u) u*nulled))
      num1s <- rowSums(c_now)

      sorted <- sort(num1s, index.return=TRUE)$ix ## I hope this is O(n)!!!!
      len <- length(sorted)
      num1sSORT <- sort(num1s)
      denoms <- (num1s*(leafSize-num1s)/leafSize)[rowSums(c_now)!=sum(nulled)]
      denomsSORT <- (num1sSORT*(leafSize-num1sSORT)/leafSize)[sum(rowSums(c_now)!=sum(nulled))]


      c_now2 <- c_now[rowSums(c_now)!=sum(nulled),]
      other_As2 <- apply(c_now2, 1, function(u) sum(Hnu[u==1])^2/norm_nu^2)

      other_Bs2 <- apply(c_now2, 1, function(u) 2*sum(Hy[u==1])*sum(Hnu[u==1])/
                          norm_nu
                        - 2*(sum(Hnu[u==1])^2%*%nut_y)/
                          norm_nu^2)

      other_Cs2 <- apply(c_now2, 1, function(u) sum(Hminus[u==1])^2)

      unus <- cumsum(Hnu[sorted])#[num1sSORT!=sum(nulled)]
      uys <- cumsum(Hy[sorted])#[num1sSORT!=sum(nulled)]
      umin <- cumsum(Hminus[sorted])#[num1sSORT!=sum(nulled)]
      ...
      indices2 <- denoms != 0
      avec <- other_As2/denoms-partialA
      bvec <- other_Bs2/denoms-partialB
      cvec <- other_Cs2/denoms-partialC
